  ...
  return (
    <div className="p-6 h-full relative overflow-hidden bg-[rgba(75,85,99,0.40)] rounded-lg border border-[rgba(255,255,255,0.10)] shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset]">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(212,207,184,0.1),transparent_50%)]" />
      </div>

      {/* Header */}
      <div className="flex items-center justify-between mb-6 relative z-10">
        <div>
          <div className="flex items-center gap-2 text-[#D4CFB8] text-sm mb-1">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <card.icon className="w-6 h-6 text-[#D4CFB8]" />
                <div>
              ...
              <div className="flex items-center gap-2">
                {card.status === 'generating' ? (
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="p-2 bg-[#D4CFB8]/20 hover:bg-[#D4CFB8]/30 rounded-lg transition-colors"
                  >
                    <IconDownload className="w-4 h-4 text-[#D4CFB8]" />
                  </motion.button>
                )}
              </div>
            </div>
      ...
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-4 relative z-10">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <IconPlayerPlay className="w-4 h-4 text-[#D4CFB8]" />
        ...
        <div className="flex items-center gap-1 mb-3">
          {Array.from({ length: 20 }).map((_, i) => {
            ...
            return (
              <motion.div
                key={i}
                className="bg-[#D4CFB8] rounded-sm"
                style={{
                  width: '3px',
                  height: heights[0],
                }}
        ...
        <div className="flex items-center justify-between">
          ...
          <motion.button
            whileHover={{ scale: 1.05 }}
            className="px-3 py-1 bg-[#D4CFB8] hover:bg-[#c2bda0] rounded-md text-[#111827] text-xs font-medium transition-colors"
          >
            Play
          </motion.button>
        </div>
      ...
      <div className="mt-4 flex justify-center">
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#D4CFB8] to-[#F8F5E4] hover:from-[#c2bda0] hover:to-[#e6e2cd] rounded-lg text-[#111827] text-sm font-medium transition-all duration-300"
        >
          <IconDownload className="w-4 h-4 text-[#111827]" />
          Export Briefing
        </motion.button>
      </div>
    </div>
